...
int deleteNode(headNode* h, int key) {
    if (h->first->key==NULL) {
        printf("비어있는 리스트입니다. 노드를 입력하고 사용해주세요.\n");
        return 0;
    }
    listNode* curt = (listNode*)malloc(sizeof(listNode));
    listNode* prev = (listNode*)malloc(sizeof(listNode));
    ...
    while (1){
        if (key == h->first->key) {
            ...
            printf("해당 리스트에 존재하지 않는 수입니다 다시 입력해 주세요.\n");
            break;
        }
        prev = prev->link;
        curt = prev->link;
 ...
 */
int invertList(headNode* h) {
    headNode* lead = (headNode*)malloc(sizeof(headNode));//이건 그냥 listNode로 받을려 생각했는데 마지막에 lesd가 Null이 되어서 혹시나 오류가 발생할까봐 바꾸었다.
    headNode* mid = (headNode*)malloc(sizeof(headNode));
    headNode* trl = (headNode*)malloc(sizeof(headNode));

    lead->first = h->first;//변수를 3개를 두어 lead에 첫번째 mid에 Null을 넣는다.
    mid->first = NULL;

    while (lead->first){//마지막에 lead가 0이 될때를 의미
        trl->first = mid->first;//trl에 mid를 복사
        mid->first = lead->first;//mid에 lead를 넣고
        lead->first = lead->first->link;//lead를 다음 노드로 이동
        mid->first->link = trl->first;//mid가 trl을 가리키도록 변경
    }
    h->first = mid->first;//마지막에 첫번째 시작 노드를 mid로 변경
    return 0;
}
